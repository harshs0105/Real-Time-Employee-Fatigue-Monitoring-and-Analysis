...
frame_check = 25   # Number of consecutive frames the EAR or MAR should be below or above threshold
...
with open(csv_file_name, mode='w', newline='') as log_file:
    ...
    while True:
        ...
        for subject in subjects:
            ...
            yawning_detected = "Yes" if mar > mar_thresh else "No"

            # Write to CSV file
            log_writer.writerow([time.strftime("%Y-%m-%d %H:%M:%S"), ear, mar, drowsiness_detected, yawning_detected, emotion])

            # Display the EAR, MAR, and emotion values on the screen
